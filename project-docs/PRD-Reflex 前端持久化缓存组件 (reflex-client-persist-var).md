# PRD: Reflex 前端持久化缓存组件 (`reflex-client-persist-var`)
## 1. 事件背景
...
## 2. 需求分析
### 2.1 目标
开发一个可独立引用的 Reflex 自定义包（`reflex_client_persist_var`），仿照 `ClientStateVar` 的操作模式，利用 TypeScript/JavaScript 桥接，为 Reflex 提供**基于浏览器 `localStorage` 的持久化前端状态管理**。
### 2.2 核心功能需求
...
### 4.1 架构与数据流向
```text
[浏览器 localStorage] <----> [全局 JS 管理器 window.__persist] <----> [React 组件渲染]
...
*   **解决思路**：设计一个隐藏的 `rx.State` 变量（如 `_persist_trigger: int`）。前端 JS 执行写入操作后，通过 `CustomEvent` 向后端发送一个极轻量的信号触发该变量自增，从而强制 React 重新渲染，读取到最新的 `localStorage` 值。
## 5. 功能规格说明 (API 设计)
### 5.1 Python 包装层 API (`PersistentVar`)
```python
# 定义变量（PersistentVar 为 frozen dataclass，必须经 create() 构造）
AppNotice = PersistentVar.create(key="app_global_notice", default_value="默认公告")
# 1. 前端读：直接绑定到 UI 组件
rx.text(AppNotice.value)
# 2. 前端写：绑定到前端事件 (如 on_change；set 为属性，不加括号)
rx.input(on_change=UserDraft.set)
# 3. 后端写前端：后端业务逻辑中调用
yield AppNotice.push("数据库最新公告")
# 4. 后端读前端：后端业务逻辑中调用，并指定回调函数（传处理器对象本身）
...
*   `window.__persist.init(key, default)`：初始化，若无值则写入默认值。
*   `window.__persist.get(key)`：读取本地值。
*   `window.__persist.set(key, value)`：写入本地，并触发 `reflex_event` 通知后端更新触发器。
*   `window.__persist.retrieve(key, callback_name)`：读取本地值，并触发 `reflex_event` 将值传给后端指定回调。
...
## 7. 边界与异常处理
1. **存储容量超限**：浏览器 `localStorage` 通常限制为 5MB。`__persist.set` 时需加入 `try...catch`，捕获 `QuotaExceededError`，并在控制台打印警告，不中断程序运行。
2. **JSON 序列化安全**：所有的值在写入前必须经过 `JSON.stringify`，读取时经过 `JSON.parse`，防止存入 `undefined` 导致解析报错。若解析失败，回退到 `default_value`。
3. **渲染时机问题**：首次渲染时若 `window.__persist` 尚未初始化，`get` 方法应返回默认值兜底，避免前端 JS 报错白屏。（实现：桥接器与各 key 的默认值注册以 `VarData` hook 形式注入页面函数体，先于首帧渲染同步执行；`init_persist_var` 挂载后的 `useEffect` 再将默认值种子写入 `localStorage`。）
## 8. 验收标准
1. **首屏秒开**：断网状态下刷新页面，页面能立刻显示上次保存的 `localStorage` 数据，不白屏。
2. **前端独立写入**：在输入框中输入内容触发 `set` 写入，`localStorage` 中的值实时更新，不产生 WebSocket 网络请求（或仅有极小的触发器更新请求），UI 正确刷新。
